<template>
  <div class="product-show">
    <v-container>
      ...
      >
        <v-progress-linear
          :active="productStore.loadingProduct"
          :indeterminate="productStore.loadingProduct"
          color="primary"
          height="4"
        ></v-progress-linear>
      </v-card>
      ...
      <div v-if="productStore.currentProduct && !productStore.loadingProduct">
            ...
            <v-card class="product-image-card" elevation="3">
              <div class="text-center pa-6">
                <v-img
                  ...
                  :src="`${appConfig.STORAGE_BASE_URL}/produits/${productStore.currentProduct.image}`"
                  ...
                  <div class="quick-stats mb-6">
                    <v-row>
                      ...
                      <v-col cols="6" md="4">
                        <div class="stat-item">
                          <div class="stat-icon">
                            <v-icon color="primary" size="24">mdi-shape</v-icon>
                          </div>
                          <div class="stat-content">
                            <div class="stat-value">{{ productStore.currentProduct.productType?.name || 'N/A' }}</div>
                      ...
                      <v-col cols="6" md="4">
                        <div class="stat-item">
                          <div class="stat-icon">
                            <v-icon color="success" size="24">mdi-star</v-icon>
                          </div>
                          <div class="stat-content">
        ...
        <!-- Variantes et Attributs Section -->
          ...
          <v-col cols="12">
            <v-card class="pa-4" elevation="3">
              <v-card-title class="text-h6 pb-4">
                ...
                <v-chip v-if="productStore.currentProduct.has_variants" color="info" variant="tonal" size="small" class="ml-2">
                  {{ productStore.currentProduct.variants?.length || 0 }} variante(s)
                </v-chip>
              </v-card-title>
                  ...
                  <v-col v-for="attribute in productStore.currentProduct.available_attributes" :key="attribute.id" cols="12" md="6">
                    <v-card variant="outlined" class="pa-3">
                      <div class="d-flex align-center mb-2">
                        ...
                        <v-chip v-if="attribute.is_required" color="error" variant="tonal" size="small" class="ml-2">
                          Requis
                        </v-chip>
                      </div>
                      <p v-if="attribute.description" class="text-body-2 text-grey-darken-1 mb-2">
                      ...
                      <div class="d-flex flex-wrap gap-1">
                        ...
                        >
                          ...
                          <span v-if="value.price_modifier && value.price_modifier != 0" class="text-success ml-1">
              ...
              <div v-if="productStore.currentProduct.has_variants && productStore.currentProduct.variants && productStore.currentProduct.variants.length > 0">
                ...
                <v-row>
                  ...
                  >
                    <v-card 
                      class="variant-card pa-4" 
                      :class="{ 'variant-inactive': !variant.is_active }"
                      elevation="2"
                    >
                      ...
                      <div class="text-center mb-3">
                        <v-img
                          v-if="variant.image"
                          :src="`${appConfig.STORAGE_BASE_URL}/variants/${variant.image}`"
                          height="120"
                          class="rounded variant-image"
                          cover
                        ></v-img>
                      ...
                      <div class="variant-info">
                        ...
                        <div class="variant-details mb-3">
                          <div class="d-flex justify-space-between align-center mb-1">
                            <span class="text-body-2 text-grey-darken-1">SKU:</span>
                            <span class="font-mono text-body-2">{{ variant.sku }}</span>
                          </div>
                          <div class="d-flex justify-space-between align-center mb-1">
                            <span class="text-body-2 text-grey-darken-1">Prix:</span>
                          ...
                          <div v-if="variant.weight" class="d-flex justify-space-between align-center mb-1">
                            <span class="text-body-2 text-grey-darken-1">Poids:</span>
                            <span class="text-body-2">{{ variant.weight }}g</span>
                          </div>
                        </div>
              ...
              <div v-if="!productStore.currentProduct.has_variants && (!productStore.currentProduct.available_attributes || productStore.currentProduct.available_attributes.length === 0)">
                <v-alert type="info" variant="tonal">
                  ...
                  Ce produit n'a pas de variantes ou d'attributs configurés.
                </v-alert>
              </div>
            </v-card>
          </v-col>
        ...
        <v-row class="mt-6">
          <v-col cols="12">
            <v-card class="pa-4" elevation="3">
              ...
              <!-- Add Review Form (if user hasn't commented yet) -->
              <div v-if="!hasUserCommented(productStore.currentProduct)" class="mb-6">
                <v-card variant="outlined" class="pa-4">
                  ...
                  <v-form @submit.prevent="submitReview">
                    ...
                    <v-textarea
                      ...
                      rows="3"
                      variant="outlined"
                      :rules="[v => !!v || 'Le commentaire est requis']"
                      class="mb-3"
                    ></v-textarea>
                    <v-btn
                      type="submit"
                      ...
                      :disabled="!reviewForm.comment || reviewForm.note === 0"
                    >
                      Publier l'avis
                    </v-btn>
                  </v-form>
                  ...
                  >
                    <v-card variant="outlined" class="pa-4">
                      <div class="d-flex align-center justify-space-between mb-3">
                        <div class="d-flex align-center">
                          <v-avatar size="40" class="mr-3">
                            <v-img
                              ...
                              :src="`${appConfig.STORAGE_BASE_URL}/avatars/${avis.user.avatar}`"
                            ></v-img>
                        ...
                        <div v-if="avis.user && avis.user.id === authStore.userAuth?.id" class="d-flex gap-2">
                          <v-btn
                            v-if="!editingReview || editingReview.id !== avis.id"
                            color="primary"
                            variant="outlined"
                            size="small"
                            @click="startEditReview(avis)"
                          >
                      ...
                      <div v-if="editingReview && editingReview.id === avis.id" class="mt-3">
                        <v-form @submit.prevent="updateReview(avis.id)">
                          ...
                          <v-btn
                            ...
                            color="success"
                            :loading="productStore.loading"
                            :disabled="!editingReview.comment || editingReview.note === 0"
                            class="mr-2"
                          >
                            Sauvegarder
                          </v-btn>
      ...
      <v-card v-else-if="!productStore.loadingProduct && !productStore.currentProduct" class="text-center pa-12" elevation="0">
        ...
        <v-btn color="primary" to="/challenger/produit">
          Retour à la liste
        </v-btn>
      </v-card>
    </v-container>
...
const hasUserCommented = (product) => {
  ...
  return product.avis.some(comment => comment.user && comment.user.id === authStore.userAuth?.id)
}
...
const submitReview = async () => {
  ...
  try {
    await productStore.commenter(productStore.currentProduct.id, {
...
const updateReview = async (reviewId) => {
  if (!editingReview.value.comment || editingReview.value.note === 0) return
  
  try {
    await productStore.editAdvice(productStore.currentProduct.id, reviewId, {
      comment: editingReview.value.comment,
      note: editingReview.value.note
    })
...
const deleteReview = async (reviewId) => {
  if (confirm('Êtes-vous sûr de vouloir supprimer cet avis ?')) {
    try {
      await productStore.supprAdvice(productStore.currentProduct.id, reviewId)
      loadProduct()
    } catch (error) {
...
// Utility methods for attributes and variants
...
/* Styles for variants and attributes */
.color-preview-tiny {
  width: 12px;
  height: 12px;
  border-radius: 50%;
